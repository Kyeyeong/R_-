...
merged_data = inner_join(seoul_map, 서울시_상권분석_데이터, 
  ...
  select(SIG_CD, 행정동_코드_명, 월평균소득, geometry)
...
result = ggplot(merged_data) + 
  ...
  labs(x='경도', y='위도', title= '서울시 22~25년 월평균 소득') +
  # geom_sf(aes(fill = 월평균소득), color = 'black') + 
  geom_sf_interactive(aes(fill=월평균소득, tooltip=행정동_코드_명, data_id=SIG_CD)) +
